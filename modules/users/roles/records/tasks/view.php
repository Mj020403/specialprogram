...
require_role(['records_officer', 'super_admin', 'system_admin', 'admin']);
$flash = get_flash();
$id = (int)($_GET['id'] ?? 0);
...
if (!$task) {
    ...
    header('Location: /harvest/records/tasks/index.php');
    exit;
}
...
page_card_start('Review Task', 'Check document details, collaborate in comments, and process the workflow step.');
flash_message($flash);
?>

<div class="flex flex-wrap gap-3 mb-6">
    <?php action_button('/harvest/records/tasks/index.php', 'Back to Pending Tasks', 'arrow-left', 'secondary'); ?>
...
<div class="rounded-3xl bg-white dark:bg-slate-900 p-6 shadow-sm ring-1 ring-slate-200 dark:ring-slate-800">
    ...
    <form action="/harvest/comments/store.php" method="POST" class="mb-5 app-form-loader">
        ...
        <input type="hidden" name="redirect_to" value="/harvest/records/tasks/view.php?id=<?= (int)$task['id'] ?>">
                ...
                <div class="rounded-2xl border border-slate-200 dark:border-slate-700 p-4">
                    <div class="flex items-start justify-between gap-3">
                        ...
                        <?php if ((int)$c['author_user_id'] === (int)$_SESSION['user_id']): ?>
                            <a href="/harvest/comments/delete.php?id=<?= (int)$c['id'] ?>&redirect_to=<?= urlencode('/harvest/records/tasks/view.php?id=' . (int)$task['id']) ?>"
