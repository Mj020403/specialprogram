...
if (in_array($role_code, ['super_admin', 'system_admin', 'admin', 'records_officer', 'approver', 'auditor', 'department_head'], true)) {
    $allowed = true;
}
...
if (!$previewable) {
    header("Location: /harvest/files/download.php?id=" . $version_id);
    exit;
}

$desc = "Previewed file version ID {$version_id}";
$stmt = $conn->prepare("
    INSERT INTO activity_logs (user_id, action_code, entity_type, entity_id, description, created_at)
    VALUES (?, 'document_file_previewed', 'document_versions', ?, ?, NOW())
");
$stmt->bind_param("iis", $user_id, $version_id, $desc);
$stmt->execute();
$stmt->close();
